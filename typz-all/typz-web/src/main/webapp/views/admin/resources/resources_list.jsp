	...
	<div class="page-container">
		...
		<div class="content content-inner">
			...
			<div class="panel-body">
				...
				<a href="${ctx }/resources/resources_edit.izhbg" id="addDialog" class="btn btn-sm default"  ><i class="fa fa-plus-square"></i>新增</a>
				<a href="javascript:resource_del()" class="btn btn-sm red" ><span class="glyphicon glyphicon-calendar"></span>删除</a>
				<a class="btn btn-sm default"  type="button" onclick="role_updflag();"><i class="fa fa-ban"></i>更改状态</a>
				<form action="${ctx}/resources/resources_list.izhbg" id="form1"></form>
			 ...
			 <table id="userGrid" class="table table-striped table-bordered table-hover dataTable" >
				...
				<tbody >
					<c:forEach items="${page.result}" var="item">
						<tr id="${item.resourceId}">
							<td style="text-align: center;" class="id">
								<input type="checkbox" name="checkdel" class="selectedItem a-check"  value="${item.resourceId}"/></td>
							<td nowrap>
									<a href="${ctx }/resources/resources_edit.izhbg?resourceId=${item.resourceId }">编辑</a>
							</td>
							...
							<td>${item.resourceString}
							</td>
							<td>${item.resourceDesc }
							</td>  
						</tr>
					</c:forEach>
				</tbody>
...
function deleteInfo(){
	$.ajax({
		type: "POST",
	    url: "${ctx }/resources/resources_dell.izhbg",
...
function changeUserState(){
	$.ajax({
			type: "POST",
		    url: "${ctx }/resources/resources_updStatus.izhbg",
		    data: $("#form1").serialize()+"&"+$("input:checkbox[name=checkdel]:checked").serialize(),
		    dataType:"html",
			cache: false,
			success: function(data){
