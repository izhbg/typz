...
var StoreBuyer = function(){
	...
	var datatableInit = function(){
		if(datatable==null){
			datatable=$('#customerInfo').DataTable({
		         ...
		         "lengthChange": false,
		         "pageLength": 4,
				 "sAjaxSource": $("#ctx").val()+"/store/goods-buyer-goodsList.izhbg",
				 "language": {
		        ...
		        "ajax" : "goods-buyer-goodsList.izhbg",
		        "columns":[
							{ "data": null},
		        ...
		        "columnDefs":[
		                      {
		                    	  ...
		                    	  "render":function(data,type,full){
		                    		  return "<input type='checkbox' id='ele_"+data.id+"' onclick='StoreBuyer.itemSelect(this)' class='selectedItem' title='"+data.name+"' value='"+data.id+"'>";
		                    	  }
		                      }
		                      ]
	...
	var itemSelect = function(e){
		if (e.checked) {
			var html = '&nbsp;<span class="label label-default" id="' + e.value + '" title="' +  e.title + '">' +  e.title + '<i class="glyphicon glyphicon-remove" style="cursor:pointer;" onclick="StoreBuyer.removeItem(this)"></i></span>';
	...
	var showDialog = function (){
	 ...
	 var deleteInfo=function(){
		$.ajax({
			type: "POST",
		    url: $("#ctx").val()+"/store/goods-buyer-delete.izhbg",
	 ...
	 var addBuyerGoods = function(){
		 ...
		 $.ajax({
				type: "POST",
			    url: $("#ctx").val()+"/store/goods-buyer-add.izhbg",
			    data: ids,
			    dataType:"html",
				cache: false,
				success: function(data){
