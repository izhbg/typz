...
var Store = function(){
	...
	var uploadIndexImg = function(obj,storeId){
		var files = obj.files;
	    var pId = $(obj).attr("id");
	    ...
	    if (files != undefined) {
            ...
            if (fs > 1 - uploadedCount) {
                alertUp("您最多能上传1张图片，请重新选择！");
                return;
            }
            ...
            $.ajaxFileUpload ({
    	        url: $("#contextpath").val()+"/storeImg/upload.izhbg?state=0&storeId="+storeId+"&r=" + Math.random(),
    	        secureuri :false,
    	        ...
    	        success: function (data,status) {
    	            ...
    	            for (i = 0; i < urls.length; i++) {
    	            	addShouIndexImgOrder(tfsBase+"/storeImg/downloaImgdFile.izhbg?attachId="+urls[i],urls[i],"im-container1");
	                }
	...
	var uploadImg = function(obj,storeId){
		var files = obj.files;
	    var pId = $(obj).attr("id");
	    ...
	    if (files != undefined) {
            ...
            $.ajaxFileUpload ({
    	        url: $("#contextpath").val()+"/storeImg/upload.izhbg?state=1&storeId="+storeId+"&r=" + Math.random(),
    	        secureuri :false,
    	        ...
    	        success: function (data,status) {
    	            ...
    	            var tfsBase = $("#contextpath").val();
    	            for (i = 0; i < urls.length; i++) {
	                    addShouImgOrder(tfsBase+"/storeImg/downloaImgdFile.izhbg?attachId="+urls[i],urls[i],"im-container2");
	                }
	...
	/*上传图片展示,填充空白位置*/
	var addShouIndexImgOrder = function(url,id,classe){
		 $('.im-container1').find('div').each(function (i, o) {
		        if ($(o).html() == '') {
		            $(o).html("<input type='hidden'/>");
		            addShouImg(url,i,id,classe);
		            return false;
		        }
		    });
	};
	/*上传图片展示,填充空白位置*/
	var addShouImgOrder = function(url,id,classe){
		$('.im-container2').find('div').each(function (i, o) {
			if ($(o).html() == '') {
				$(o).html("<input type='hidden'/>");
				addShouImg(url,i,id,classe);
				return false;
	...
	var addShouImg = function(url, i,id,classe){
       ...
       $("#add-is-big"+i).load(function () {
            ...
            $('.'+classe).find('div').eq(i).html(htmlData);
            $('.'+classe).find('div').eq(i).removeClass('no-imgs');
            $(this).parent().remove();
	...
	return{
		...
		uploadImg:function(obj,storeId){
			uploadImg(obj,storeId);
		},
		uploadIndexImg:function(obj,storeId){
			uploadIndexImg(obj,storeId);
		},
		initJcrop:function(){
			initJcrop();
		},
